...
export function Services() {
  ...
  return (
    <section className="mx-auto w-[88vw] max-w-[1100px] py-16 md:py-24">
      <div className="mb-8 md:mb-9" data-aos="fade-up">
        <div>
          ...
          <TypingHeading
            ...
            className="hero-display mt-3 whitespace-normal text-2xl font-bold text-white md:whitespace-nowrap md:text-4xl"
          />
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-8">
            ...
            return (
              <button
                ...
                >
                  ...
                  <Icon className={cn("h-3.5 w-3.5 shrink-0 md:h-4 md:w-4", selected ? "text-cyan-200" : "text-[#8892b0]")} />
                </span>
                <span className="mt-1 block text-xs font-semibold text-[#ccd6f6] md:text-sm">
        ...
        <motion.article
          ...
          className="min-h-[330px] md:min-h-[360px]"
        >
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex min-w-0 flex-wrap items-center gap-3">
              ...
              <span className="hero-mono text-xs font-bold text-[#ccd6f6] md:text-sm">
          ...
          <h3 className="mt-6 text-xl font-bold leading-7 text-[#ccd6f6] md:mt-8 md:text-2xl md:leading-8">
            {active.title}
          </h3>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-[#a8b2d1] md:mt-5 md:text-base md:leading-8">
            {active.proof}
          </p>

          <ul className="mt-5 grid gap-3 md:mt-7">
            {active.details.map((detail) => (
              <li key={detail} className="grid grid-cols-[16px_1fr] gap-2 text-sm leading-6 text-[#a8b2d1] md:grid-cols-[18px_1fr] md:gap-3 md:leading-7">
          ...
          <div className="mt-6 md:mt-7">
            ...
            <div className="mt-3 flex flex-wrap gap-2 md:mt-4 md:gap-3">
              {active.projects.map((project) => (
                <span
                  key={project}
                  className="rounded border border-[#233554] bg-[#112240]/70 px-3 py-2 text-xs font-semibold text-[#ccd6f6] md:px-4 md:py-3 md:text-sm"
                >
                  {project}
                </span>
              ))}
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2 md:mt-8 md:gap-3">
            {active.skills.map((skill) => (
              <span key={skill} className="grid grid-cols-[14px_1fr] gap-2 text-xs leading-5 text-[#a8b2d1] md:grid-cols-[16px_1fr] md:gap-3 md:text-sm md:leading-6">
                <span className="mt-1.5 h-0 w-0 border-y-[4px] border-l-[6px] border-y-transparent border-l-cyan-300 md:mt-2" />
                {skill}
              </span>
            ))}
          </div>
